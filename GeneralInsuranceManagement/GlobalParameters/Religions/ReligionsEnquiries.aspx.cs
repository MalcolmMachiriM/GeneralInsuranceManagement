...
{
    public partial class Religions : System.Web.UI.Page
    {
        ...
        {
            Models.GlobalParameters.Religions religion = new Models.GlobalParameters.Religions("cn", long.Parse(txtuserid.Value));
            string sql = "select * from Religions";
            DataSet ds = religion.GetUsers(sql);
            if (ds != null)
            {
            ...
            else
            {
                ...
                grdAcctypes.DataBind();
                WarningAlert("No Religions found");
            }
        }
        ...
        protected void grdAcctypes_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                Response.Redirect(string.Format("~/GlobalParameters/Religions/ReligionsCreate?ReligionsId=" + Id, Id), false);
            }
            if (e.CommandName == "deleteRecord")
            {
                Models.GlobalParameters.Religions religions = new Models.GlobalParameters.Religions("cn", 1);
                if (religions.Retrieve(long.Parse(Id)))
                {
                    if (religions.Delete())
                    {
