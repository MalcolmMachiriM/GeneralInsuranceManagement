...
namespace GeneralInsuranceManagement.Models.GlobalParameters
{
    public class Religions
    {
        ...
        public Religions(string ConnectionName, long ObjectUserID)
        {
        ...
        {
            DbCommand cmd = db.GetStoredProcCommand("sp_Save_Religions");
        ...
        {
            string text = ID <= 0 ? "SELECT * FROM Religions WHERE ID = " + mId : "SELECT * FROM Religions WHERE ID = " + ID;
        ...
        protected virtual bool Retrieve(string sql)
        {
            try
            {
                ...
                SetErrorDetails("Religion not found.");
                return false;
            }
            catch (Exception ex)
            {
        ...
        public virtual bool Delete()
        {
            ...
            return Delete("DELETE FROM Religions WHERE ID = " + mId);
