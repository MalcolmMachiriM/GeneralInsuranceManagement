...
namespace GeneralInsuranceManagement.Models.GlobalParameters
{
    public class InterestRateType
    {
        ...
        public DateTime DateaModified { get { return mDateModified; }set { mDateModified = value; } }
        public Database Database => db;

        public string OwnerType => GetType().Name;
        public string ConnectionName => mConnectionsName;
        public InterestRateType(string ConnectionName, long ObjectUserID)
        {
        ...
        {
            DbCommand cmd = db.GetStoredProcCommand("sp_Save_InterestRateTypes");
        ...
        {
            string text = ID <= 0 ? "SELECT * FROM InterestRateTypes WHERE ID = " + mId : "SELECT * FROM InterestRateTypes WHERE ID = " + ID;
        ...
        protected virtual bool Retrieve(string sql)
        {
            try
            {
                ...
                SetErrorDetails("Interest Rate Type not found.");
                return false;
            }
            catch (Exception ex)
            {
        ...
        public virtual bool Delete()
        {
            ...
            return Delete("DELETE FROM InterestRateTypes WHERE ID = " + mId);
