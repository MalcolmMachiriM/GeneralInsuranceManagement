...
namespace GeneralInsuranceManagement.Models.GlobalParameters
{
    public class HabitsAndInterests
    {
        ...
        {
            DbCommand cmd = db.GetStoredProcCommand("sp_Save_HabitsAndInterests");
        ...
        {
            string text = ID <= 0 ? "SELECT * FROM HabitsAndInterests WHERE ID = " + mId : "SELECT * FROM HabitsAndInterests WHERE ID = " + ID;
        ...
        protected virtual bool Retrieve(string sql)
        {
            try
            {
                ...
                SetErrorDetails("Habit or Interest not found.");
                return false;
            }
            catch (Exception ex)
            {
