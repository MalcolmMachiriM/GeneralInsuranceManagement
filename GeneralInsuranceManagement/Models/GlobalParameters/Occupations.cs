...
namespace GeneralInsuranceManagement.Models.GlobalParameters
{
    public class Occupations
    {
        ...
        public Occupations(string ConnectionName, long ObjectUserID)
        {
        ...
        {
            DbCommand cmd = db.GetStoredProcCommand("sp_Save_Occupations");
        ...
        {
            string text = ID <= 0 ? "SELECT * FROM Occupations WHERE ID = " + mId : "SELECT * FROM Occupations WHERE ID = " + ID;
        ...
        protected virtual bool Retrieve(string sql)
        {
            try
            {
                ...
                SetErrorDetails("Occupation not found.");
                return false;
            }
            catch (Exception ex)
            {
        ...
        public virtual bool Delete()
        {
            ...
            return Delete("DELETE FROM Occupations WHERE ID = " + mId);
